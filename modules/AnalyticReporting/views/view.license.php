...
class AnalyticReportingViewLicense extends SugarView
{
    ...
    protected function _getModuleTitleParams($browserTitle = false)
    {
        ...
        return array(
           ...
           translate('LBL_ANALYTICREPORTING','Administration').': '.translate('LBL_ANALYTICREPORTING_LICENSE_TITLE','Administration'),
           );
    }
    ...
    {
        global $current_user;
        // #5793 Display an error message when licensing fails, for both admin and regular users who can not access administrative features
        if(isset($_REQUEST['inactive'])){
            echo ('<h2><p class="error">AnalyticReporting is inactive</p></h2><p class="error">'.urldecode($_REQUEST['inactive']).'</p>');
        }
    ...
    public function display()
    {
        ...
        include('modules/'.$currentModule.'/license/config.php');
        ...
        $license_strings = AnalyticReportingViewLicense::loadLicenseStrings();
        ...
        if(isset($outfitters_config['manage_licensed_users']) && $outfitters_config['manage_licensed_users'] == true) {
            ...
            if(!empty($trimmed_last))
            {
                ...
                $validated = AnalyticReportingOutfittersLicense::doValidate($currentModule);
