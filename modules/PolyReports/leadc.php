...
class Leadc {
    function getAllLeadOfSecurityGroupAndPerTelesale($list_telesale = [],$from,$to,$dot_nhap_hoc,$nodate){
        ...
        if(strlen($dot_nhap_hoc)>2 && isset($dot_nhap_hoc)){
            ...
            // var_dump('không có đợt nhập học');
            if($nodate=='all'){
            ...
            // var_dump('không có ngày');
                
            }else{
                //Tìm lead theo ngày được nhâp vào
        ...
        // var_dump('<p> số lead' . count($data) . '</p>');
        // Common::dd($data);
    ...
    function getAllLeadActiveOfSecurityGroupAndPerTelesale($list_telesale = [],$from,$to,$dot_nhap_hoc,$nodate){
        ...
        if(strlen($dot_nhap_hoc)>2 && isset($dot_nhap_hoc)){
            if($nodate=='all'){
                $sql = "SELECT a.status,a.assigned_user_id,b.dot_nhap_hoc_c,a.converted FROM leads a,leads_cstm b  WHERE a.deleted=0 AND a.id = b.id_c AND a.status = 'active_lead' AND b.dot_nhap_hoc_c  IN ($str_dot_nhap_hoc) AND a.assigned_user_id IN ($list_telesale_str)";
            ...
            if($nodate=='all'){
                $sql = "SELECT a.status,a.assigned_user_id,b.dot_nhap_hoc_c,a.converted  FROM leads a,leads_cstm b  WHERE a.deleted=0 AND a.id = b.id_c AND a.status = 'active_lead' AND a.assigned_user_id IN ($list_telesale_str)";

            }else{
                $sql = "SELECT a.status,a.assigned_user_id,b.dot_nhap_hoc_c,a.converted,a.id FROM leads a,leads_cstm b  WHERE a.deleted=0 AND b.date_activated_c>='$begin' AND b.date_activated_c<'$end' AND a.id = b.id_c AND a.assigned_user_id IN ($list_telesale_str)";
        ...
        $data = [];
        while($row = $result->fetch_assoc()) {
            array_push($data,(object) ["status"=>$row['status'],"assigned_user_id"=>$row["assigned_user_id"],"dot_nhap_hoc_c"=>$row["dot_nhap_hoc_c"],"converted"=>$row["converted"],"id"=>$row["id"]]);
        }

        // Common::dd($data);
  
        // var_dump('<p> số active lead' . count($data) . '</p>');
        // var_dump('<pre>');
        // foreach($data as $p){
        //     echo '<p>';
        //     var_dump($p);
    ...
    function getReportLeadPerTelesale($list_telesale = [],$from,$to,$dot_nhap_hoc,$nodate){
        ...
        //     echo '</p>';
        // }
        // var_dump([$from,$to,$dot_nhap_hoc,$nodate]);
