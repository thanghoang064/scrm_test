...
include_once 'modules/PolyReports/voice.php';

class PolyReportsViewVoice extends SugarView {
    ...
    public function display() {
        ...
        if($access_security){
                ...
                if(isset($_REQUEST['telesale'])){
                    switch ($_REQUEST['telesale']) {
                        ...
                        default:
                            ...
                            $report_per_telesale= $voice->getReportCallPerTelesale([$this_telesale],$from,$to);
                            $ss->assign('telesale',$_REQUEST['telesale']);
                        
                    };
                }else{
                    $html = $ss->fetch('modules/PolyReports/tpls/voice.tpl');   
                    // die;
                }
       
                $builder_report = $voice->reportBuilderData($report_per_telesale,$from,$to);
            ...
            $html = $ss->fetch('modules/PolyReports/tpls/voice-report.tpl');
            // die;
        }else{
            $ss->assign('security_groups', $security_groups);
            $html = $ss->fetch('modules/PolyReports/tpls/voice.tpl');  
