...
class VoiceAll  {
    ...
    function convertSecondToFullTimeTextAll($seconds){
        $result = gmdate("d- H:i:s", $seconds);
        // $result=$seconds;
      return $result;
    }
    ...
    //    var_dump($call_records);die;
        ...
        foreach ($list_telesale as $key=>$telesale){
            ...
            if( count($list_record_of_user)==0 && count($calls)==0){
                ...
                //    Common::dd($list_record_of_user);
                $report->data=[(object)['call_held'=>$call_held,'total_call'=>$total_call,'record_calls'=>$record_calls,'not_held'=>$total_call-$call_held,'duration_held'=> $this->convertSecondToFullTimeText($duration_held),'rate'=> $this->convertSecondToFullTimeText($rate),'try_to_contact'=>$try_to_contact,'active_lead'=>$active_lead,'recall'=>$recall,'not_interested'=>$not_interested,]];
        ...
        // $swap=[];
        // for($i=0;$i<count($my_report);$i++){
        //     for($j=0;$j<count($my_report);$j++){
        //         if($i != $j){
        //             // if($my_report[$i])
        //             // var_dump($my_report[$i]->report->data);
        //             // var_dump($my_report[$i]->report->data[0]->active_lead);

        //             if(count($my_report[$i]->report->data) !=0 && count($my_report[$j]->report->data ) !=0){
        //                 if($my_report[$i]->report->data[0]->active_lead < $my_report[$j]->report->data[0]->active_lead){
        //                     $swap = $my_report[$i];
        //                     $my_report[$i] = $my_report[$j];
        //                     $my_report[$j] = $swap;
        //                     // $i = 0 ;
        //                     // $j = 0;
        //                 }
        ...
        //     }
        // }
        $count_report = count($my_report);
        // Lặp để sắp xếp
        for ($i = 0; $i < $count_report - 1; $i++)
        {
            // Tìm vị trí phần tử lớn nhất
            $max = $i;
            for ($j = $i + 1; $j < $count_report; $j++){
                if(count($my_report[$i]->report->data) !=0 && count($my_report[$j]->report->data ) !=0){
                    if ($my_report[$j]->report->data[0]->active_lead > $my_report[$max]->report->data[0]->active_lead){
                        $max = $j;
                    }
                }
            }
            ...
            // với vị trí thứ $i
            $temp = $my_report[$i];
            $my_report[$i] = $my_report[$max];
            $my_report[$max] = $temp;
        }
        // foreach($my_report as $report){
        //     if(count($report->report->data) !=0){
                
        //         var_dump($report->report->data[0]->active_lead);
        ...
        $report->data=[(object)['call_held'=>$total_held,'total_call'=>$total_voice,'record_calls'=>$total_calls,'not_held'=>$total_voice-$total_held,'duration_held'=> $this->convertSecondToFullTimeText($total_duration),'rate'=> $this->convertSecondToFullTimeText($total_rate),'duration_held_raw'=> $total_duration,'rate_raw'=> $total_rate,'try_to_contact'=>$total_try_contact,'active_lead'=>$total_active_lead,'recall'=>$total_recall,'not_interested'=>$total_not_interested]];
        if(count($report->data)== 0 ){
            $report->empty = 0;
        }
        array_push($my_report, (object)['report'=>$report]);
