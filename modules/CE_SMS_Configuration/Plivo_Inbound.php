...
if($validate == 1) {
    ...
    global $timedate;
if (!empty($_REQUEST['From']) && !empty($_REQUEST['Text'])) {
    ...
    $Contact_Query = "SELECT *  FROM contacts WHERE phone_home LIKE '%" . $From . "%' OR phone_mobile LIKE '%" . $From . "%' OR phone_work LIKE '%" . $From . "%' OR phone_other LIKE '%" . $From . "%' OR phone_fax LIKE '%" . $From . "%' Limit 1";
    ...
    $New_Call->description = $_REQUEST['Text'];
    ...
    if (!empty($Save_Call)) {
        ...
        $alert->name = $subject;
        $alert->description = $_REQUEST['Text'];
    ...
    $GLOBALS['log']->fatal('Plivo Inbound SMS Failed.');
}
