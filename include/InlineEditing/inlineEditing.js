...
function handleSave(field, id, module, type) {
    ...
    var phoneArr = ["phone_mobile", "phone_work", "phone_home", "phone_other"];
    if (field === "email1") {
        setValueClose(output_value, false);
    } else if( phoneArr.includes(field) ) {
        var resp = output_value;
        try {  
            const json = JSON.parse(resp);
            if(json.status == false){
                let reload = confirm(
                    `Lưu không thành công: ${json.msg} \nBạn có muốn tải lại trang hay không?
                `);
                if(reload === true){
