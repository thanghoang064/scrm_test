...
echo "
<script>
	sugarListView.prototype.check_custom_list2 = function(form, field, value, pageTotal) {
        count = 0;
		$('.listview-checkbox').attr('checked', false);
		var data= [];
		$('input.listview-checkbox').each(function (i) {
			data[i]=$(this).val();
		});
		
        for (k = 0; k < pageTotal; k++) {
            count++;
			$('input[value=\"'+data[k]+'\"]').prop( 'checked', true );
		...
		$menuItems = array(
				"<label class=\"hidden glyphicon bootstrap-checkbox glyphicon-unchecked\"><span class='suitepicon suitepicon-action-caret'>
				</span></label>
				<input title=\"".$app_strings['LBL_SELECT_ALL_TITLE']."\" type='checkbox' class='bootstrap-checkbox-hidden checkbox massall' name='massall' id='massall_".$location."' value='' onclick='sListView.check_all(document.MassUpdate, \"mass[]\", this.checked);' /> $selectObjectSpan
				...
				<input style='color:#fd4936;line-height: 0px;height: 25px;' type='number' id='nbselect' placeholder='Nhập số bản ghi'  
				value='' onchange='sListView.check_custom_list2(document.MassUpdate, \"mass[]\",1,this.value);' >
