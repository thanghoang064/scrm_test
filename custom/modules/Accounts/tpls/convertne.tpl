				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<p>Dân tộc hiện tại: <span
									style="font-style: italic;font-weight:400;">{$record->ethnicity_c}</span></p>
						</td>
					...
					<tr>
						...
						<td><input type="text" id="id_passport" value="{$record->id_passport_c}"
					...
					<tr>
						...
						<td>
							<select class="select2" name="place_issued" id="place_issued" style="max-width:200px;">
								<option value=""></option>
							</select>
							<p>Nơi cấp hiện tại: <span
									style="font-style: italic;font-weight:400;">{$record->place_issued_c}<span></p>
							</span>
						</td>
						...
						<td>
							<select id="time_study">
								...
								<option value="evenning" {if $record->study_time_c == 'evenning'}selected{/if}>Ban đêm
								</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<p>Tỉnh,Thành phố hiện tại: <span
									style="font-style: italic;font-weight:400;">{$record->province_name}<span></p>
						</td>
						...
						<td><select name="district" style="max-width:200px;" id="district">
							...
							<p>Quận huyện hiện tại: <span
									style="font-style: italic;font-weight:400;">{$record->district_c}<span></p>
						</td>
					</tr>
					<tr>
						...
						<td><select class="select2" name="school" style="max-width:200px;" id="school">
							...
							<p>Trường hiện tại: <span
									style="font-style: italic;font-weight:400;">{$record->school_c}<span></p>
						</td>
						...
						<td>
							...
							- Năm tốt nghiệp <input style="width: 55px;" id="year_graduated"
								value="{$record->year_graduated_c}" onkeypress="return isNumber(event)">
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							...
							<p>Khóa nhập học hiện tại: <span
									style="font-style: italic;font-weight:400;">{$record->campaign_name}<span></p>
						</td>
					</tr>
					<tr>
						<td><b>Email: </b></td>
						...
						<td>
							...
							<p>Ngành: <span style="font-style: italic;font-weight:400;" id="sector"></span></p>
						</td>
					...
					<tr>
						...
						<td><input type="text" id="phone1" value="{$record->phone1_c}"
								onkeypress="return isNumber(event)" maxlength="10"></td>
						<td><b>SĐT sinh viên (phụ):</b></td>
						<td><input type="text" id="phone3" value="{$record->phone3_c}"
								onkeypress="return isNumber(event)" maxlength="10"> </td>
					</tr>
					<tr>
						<td class="required2 phone2"><b>SĐT phụ huynh (chính): </b></td>
						<td><input type="text" id="phone2" value="{$record->phone2_c}"
								onkeypress="return isNumber(event)" maxlength="10"></span></td>
						<td><b>SĐT phụ huynh (phụ):</b></td>
						<td><input type="text" id="phone4" value="{$record->phone4_c}"
								onkeypress="return isNumber(event)" maxlength="10"></td>
					</tr>
					<tr>
						<td><b>Quốc gia: </b></td>
		...
		<div>
			...
			<div>
				<script type="text/javascript">
					let race_now = '{$record->ethnicity_c}';
					let place_issued_now = '{$record->place_issued_c}';
					let province_name_now = '{$record->province_name}';
					let school_name_now = '{$record->school_c}';
					let district_name_now = '{$record->district_c}';
					{ literal }
					var store_data = [];
					var now_campus = 0;
					var gl_sectors_id = 0;
					...
					$.ajax({
						type: 'GET',
						url: `${crm_domain}/crm/get_list.php?usr=${acc.username}`,
						dataType: "json",
						success: function (data, status) {
							...
							if (data.type == "success") {
								...
								$.each(data.data.race, function (index, value) {
									...
									$("select[name='race']").append(`<option value='${value.id}' ${value.id == race_now ? 'selected' : ''}>${value.name}</option>`);
								});
								...
								let id_province = false;
								$.each(data.data.province, function (index, value) {
									const check = value.name == province_name_now;
									if(check){
										id_province = value.id
									}
									$("select[name='province']").append(`<option value='${value.id}' ${value.name == province_name_now ? 'selected' : ''}>${value.name}</option>`);
								});
								$.each(data.data.province, function (index, value) {
									$("select[name='place_issued']").append(`<option value='${value.name}' ${value.name == place_issued_now ? 'selected' : ''}>${value.name}</option>`);
								});
								...
								$.each(data.data.district, function (index, value) {
									if(id_province){
										if(value.province_id == id_province){
											$("select[name='district']").append(`<option value='${value.id}' ${value.name == district_name_now ? 'selected' : ''}>${value.name}</option>`);
										}
									}else{
										$("select[name='district']").append(`<option value='${value.id}' ${value.name == district_name_now ? 'selected' : ''}>${value.name}</option>`);
									}
								...
								$("select[name='province']").on("change", function (e) {
									...
									if (now_province == '' || now_province == undefined || now_province == null) {
										...
										$.each(store_data.schools, function (index, value) {
											if (value.name.search(school_name_now) >= 0 && checkSchool == 0) {
												$("select[name='school']").append(`<option value='${value.id}' selected>${value.name}</option>`);
												checkSchool = 1;
											} else {
												$("select[name='school']").append(`<option value='${value.id}'>${value.name}</option>`);
											}
										...
										$.each(store_data.schools, function (index, value) {
											if (value.province_id == now_province) {
												if (value.name.search(school_name_now) >= 0 && checkSchool2 == 0) {
													$("select[name='school']").append(`<option value='${value.id}' selected>${value.name}</option>`);
													checkSchool2 = 1;
												} else {
													$("select[name='school']").append(`<option value='${value.id}'>${value.name}</option>`);
								...
								$("select[name='branch']").change(function () {
									...
									let now_branch = $(this).val();
									let sector_id = data.data.branch.find((item) => {
										return item.id == now_branch;
									})['sectors_id'];
									let sector = data.data.sectors.find((item) => {
										return item.id == sector_id;
									})['name'];
									$("#sector").text(sector);
									gl_sectors_id = sector_id;
								});
					...
					$("#convert").click(function () {
						...
						var first_name_raw = people.first_name;
						var n = first_name_raw.split(" ");
						people.first_name = n[n.length - 1];
						...
						people.sectors_id = gl_sectors_id
