...
class FUMailAuthenticateUser extends SugarAuthenticateUser {
    ...
    function authenticateUser($name, $password, $fallback = false) {
        $row = ExtUser::findUserWithoutPassword($name, "(portal_only IS NULL OR portal_only !='1') AND (is_group IS NULL OR is_group !='1') AND status !='Inactive'");
