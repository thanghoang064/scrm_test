...
class ViewConvertLead extends SugarView
{
    ...
    protected function handleSave()
    {
        ...
        foreach ($beans as $bean)
        {
            if (!empty($lead))
            {
				...
				{
					$bean->birthdate_c = $lead->birthdate;
					$bean->district_c = $lead->primary_address_state;
					$bean->email_c = $lead->email_c;
					$bean->phone1_c = $lead->phone_mobile;
					$bean->phone2_c = $lead->phone_home;				
					$bean->phone3_c = $lead->phone_work;
					$bean->phone4_c = $lead->phone_other;
					$bean->address_c = $lead->jjwg_maps_address_c;
					$bean->nguon_c = $lead->source_c;
					$bean->promoter_c = $lead->created_by;
					$bean->source_c = $lead->lead_source;
                    $bean->province_c = $this->checkProvince($lead->primary_address_city);
                    $bean->year_graduated_c = $lead->graduation_year_c;
                    $bean->place_issued_c = $lead->place_of_issue_c;
                    $bean->date_issued_c = $lead->issue_date_c;
                    $bean->gender_c = ($lead->gender_c == 'Nam') ? 'male' : 'female';
                    $bean->ethnicity_c = strtolower($lead->ethnicity_c);
        ...
        {	//Mark the original Lead converted
            ...
            $lead->save();
			$GLOBALS['log']->fatal("Convert Leads API"); 
			$result=CreateAccountAPI($lead);
    ...
    protected function checkProvince($name){
        $arrayProvince = [
            411 => 'Bắc Giang',
            412 => 'Bắc Kạn',
            413 => 'Bắc Ninh',
